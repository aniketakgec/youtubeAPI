...
async function formSearch(e) {
  ...
  try {
    const apidata = await fetch(
      `https://www.googleapis.com/youtube/v3/search?key=${API_KEY}&type=video&part=snippet&maxResults=50&q=${searchValue}`
    );
...
async function displayVideos(arr, item, videoDataStatistics) {
  ...
  var viewCount = getViewCount(
    videoDataStatistics.items[0].statistics.viewCount
  );
...
async function fetchSingleVideo(arr, item) {
  try {
    const videoData = await fetch(
      `https://www.googleapis.com/youtube/v3/videos?key=${API_KEY}&id=${item.id.videoId}&part=snippet,statistics`
    );
